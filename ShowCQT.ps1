...
# Is audio copy and vp9 in mkv container, you can change it if you want.
...
if ($audio -match '\\$') {
    ...
    foreach ($audio_file in Get-ChildItem -Exclude cover.*) {
        ...
        ffmpeg.exe -hide_banner -loop 1 -i $image -i $audio_file -filter_complex $filter -map '[vo]' -map '1:a' -c:a copy -c:v libvpx-vp9 $output_file
    }
}
else {
    $output_file = $audio -replace '.flac|.wav|.mp3|.m4a','.mkv'
    ffmpeg.exe -hide_banner -loop 1 -i $image -i $audio -filter_complex $filter -map '[vo]' -map '1:a' -c:a copy -c:v libvpx-vp9 $output_file
}
